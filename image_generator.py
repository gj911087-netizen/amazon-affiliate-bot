...
BENEFICIOS = ["✅ Calidad", "⚡ Fácil uso", "🎨 Moderno", "🏆 Top ventas"]
EMOJIS_FLOAT = ["❤️", "😍", "👍", "💥", "⭐", "🔥", "😮", "💯"]
...
def _draw_brand(draw, alpha=255):
    ...
    font_logo = _font(28)
    draw.text((57, 57), "T", font=font_logo, fill=(255,255,255,255), anchor="mm")
    # Texto marca
    font_b = _font(18)
    font_s = _font_r(14)
...
def _draw_emojis(draw, alpha=255, frame=0):
    emojis = random.sample(EMOJIS_FLOAT, min(len(EMOJIS_FLOAT), len(EMOJI_POSITIONS)))
    font   = _font(52)
    for i, (ex, ey) in enumerate(EMOJI_POSITIONS[:len(emojis)]):
        # Flotación suave
        fy = ey + int(8 * math.sin((frame * 0.15) + i * 1.2))
        a  = min(alpha, 220)
        draw.text((ex, fy), emojis[i % len(emojis)], font=font,
...
def _draw_hook_text(draw, hook, alpha=255):
    ...
    # Subtítulo
    f_sub = _font_r(24)
    draw.text((52, 355), "Lo más vendido esta semana", font=f_sub,
...
def _paste_product(canvas, prod_img, scale=1.0, float_y=0, glow=True):
    ...
    if glow:
        acc = PALETA["acc"]
        gl  = Image.new("RGBA", (pw+160, ph+160), (0,0,0,0))
        gd  = ImageDraw.Draw(gl)
        for r in range(80, 0, -5):
            a = int(40*(1-r/80))
            gd.ellipse([80-r, 80-r+ph//6, pw+80+r, ph+80-ph//6+r], fill=(*acc,a))
        gl = gl.filter(ImageFilter.GaussianBlur(22))
        canvas.paste(gl, (px-80, py-80), gl)

        # Sombra inferior
        sh = Image.new("RGBA", (pw+100, 70), (0,0,0,0))
        sd = ImageDraw.Draw(sh)
        sd.ellipse([10,10,pw+90,60], fill=(0,0,0,100))
        sh = sh.filter(ImageFilter.GaussianBlur(18))
        canvas.paste(sh, (px-50, py+ph-20), sh)
