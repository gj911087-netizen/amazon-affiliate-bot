...
# ── Historial en memoria (se reinicia con cada deploy) ───
posted_asins = set()

def already_posted(asin):
    return asin in posted_asins

def mark_posted(asin):
    posted_asins.add(asin)
    print(f"📦 Total publicados esta sesión: {len(posted_asins)}", flush=True)
    # Reiniciar historial si llega a 500
    if len(posted_asins) >= 500:
        posted_asins.clear()
